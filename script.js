...
const GEMINI_API_KEY = "AIzaSyCZsaJgXyUbysJrybJc-7owu0i7nM81NLI"; 
...
async function askGeminiAI(chatMessages) {
    showTypingIndicator();
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error("Деталі помилки Gemini API:", errorBody);

            if (response.status === 429) {
                addMessage("Limit request", "bot");
            } else {
                addMessage(`Connection lost. (Status: ${response.status})`, "bot");
            }
            return;
        }
